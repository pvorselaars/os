...
uint64_t kernel_stack[512] = {[0] = 0x1};
uint64_t gp_stack[512] = {[0] = 0x2};
uint64_t df_stack[512] = {[0] = 0x3};
uint64_t pf_stack[512] = {[0] = 0x4};
...
void interrupt_init()
{
	...
	tss.ist1 = (uint64_t)&gp_stack[511];
	tss.ist2 = (uint64_t)&pf_stack[511];
	tss.ist3 = (uint64_t)&df_stack[511];
	...
	register_interrupt(idt, 0x8, CODE_SEG, interrupt8, KERNEL, INTERRUPT_GATE, 3);
	register_interrupt(idt, 0xD, CODE_SEG, interrupt13, KERNEL, INTERRUPT_GATE, 1);
	register_interrupt(idt, 0xE, CODE_SEG, interrupt14, KERNEL, INTERRUPT_GATE, 2);
