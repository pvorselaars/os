...
idt_descriptor idtr;
tss64 tss = {.rsp0 = 0x1000}; // Non-zero init to force .data section

uint64_t kernel_stack[512] = {[0] = 0x1}; // Non-zero init to force .data section
...
void interrupt_init()
{
	...
	register_interrupt(idt, 0x8, CODE_SEG, interrupt8, KERNEL, INTERRUPT_GATE, 0);
